...
export default function MasterCoursesEditor() {
  ...
  const openModal = (index = null) => {
    ...
    if (index !== null) {
      ...
      setEditIndex(null);
      setTempCourse({ ...EMPTY_COURSE, order: courses.length, points: [""], paymentInfo: [""] });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSaveCourse = async () => {
    ...
    try {
      ...
      const courseToSave = {
        ...tempCourse,
        order: tempCourse.order !== undefined && tempCourse.order !== "" ? Number(tempCourse.order) : (editIndex !== null ? 0 : courses.length)
      };

      if (editIndex !== null) {
        updated[editIndex] = courseToSave;
      } else {
        updated.push(courseToSave);
      }

      // Clean up empty strings and sort by order before saving
      const cleanedCourses = updated
        .map(c => ({
          ...c,
          order: Number(c.order ?? 0),
          points: c.points.map(p => p.trim()).filter(p => p !== ""),
  ...
  const updateCourseOrder = async (index, newOrder) => {
    const itemsCount = courses.length;
    ...
    // Re-index all order values sequentially
    const cleanedCourses = updatedItems.map((c, idx) => ({
      ...c,
      order: idx,
    }));
    ...
    try {
      await putData({ courses: cleanedCourses });
      addToast("Course order updated successfully!", "success");
    } catch (err) {
      setCourses(previousCourses);
                ...
                {courses.length === 0 ? (
                  ...
                  courses.map((course, index) => (
                    <tr key={index}>
                      ...
                      <td style={{ maxWidth: "300px", wordBreak: "break-word", whiteSpace: "normal" }}>{course.name}</td>
                      <td>{course.fees}</td>
                      <td style={{ textAlign: "center" }}>
                        <select
                          ...
                          value={index}
                          onChange={(e) => updateCourseOrder(index, parseInt(e.target.value))}
                          disabled={saving}
                        >
                          {courses.map((_, idx) => (
            ...
            <div className={styles.modalBody}>
              <div className={styles.grid}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <input
                    type="number"
                    value={tempCourse.order ?? 0}
                    onChange={(e) => setTempCourse((p) => ({ ...p, order: e.target.value === "" ? "" : Number(e.target.value) }))}
                    placeholder="e.g. 0"
                  />
                </div>
