...
const Navbar = ({ initialData = [] }) => {
  ...
  const isHome = normPathname === "/" || normPathname === "/home";

  // Returns " active" string if the path matches current route
  const isActive = (path) => {
    if (!mounted) return "";
    const normPath = path.replace(/\/$/, "") || "/";
    if (normPath === "/" || normPath === "/home") {
      return normPathname === "/" || normPathname === "/home" ? " active" : "";
    }
    return normPathname.startsWith(normPath) ? " active" : "";
  };
