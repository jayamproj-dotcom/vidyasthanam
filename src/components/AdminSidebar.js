  ...
  return (
    <aside className={`${styles.sidebar} ${isOpen ? styles.sidebarOpen : ""}`}>
      ...
      <div className={styles.menuList}>
        ...
        <Link
          href="/admin/navbar"
          className={`${styles.menuItem} ${mounted && normPathname === "/admin/navbar" ? styles.activeMenu : ""}`}
          onClick={onClose}
        >
        ...
        <Link
          href="/admin/settings"
          className={`${styles.menuItem} ${mounted && normPathname === "/admin/settings" ? styles.activeMenu : ""}`}
          onClick={onClose}
          style={{ marginBottom: "10px" }}
        >
          <i className="fas fa-cogs"></i>
          <span>SMTP & Social Media</span>
        </Link>
        ...
        {/* Home Dropdown */}
        <div
          className={`${styles.menuItem} ${mounted && normPathname.startsWith("/admin/home") ? styles.activeMenu : ""}`}
          onClick={() => {
            setIsHomeOpen(!isHomeOpen);
            if (normPathname === "/admin") router.push("/admin/home/metadata");
          }}
        >
          <div
            style={{
          ...
          <i
            className={`fas ${isHomeOpen ? "fa-chevron-down" : "fa-chevron-right"}`}
            style={{ fontSize: "12px" }}
          ></i>
        </div>
          ...
          >
            {homeSubPages.map((sub) => (
              <Link
                key={sub.name}
                href={sub.path}
                className={`${styles.menuItem} ${mounted && normPathname === sub.path ? styles.activeMenu : ""}`}
        ...
        {otherPages.map((page) => (
          <Link
            key={page.name}
            href={page.path}
            className={`${styles.menuItem} ${mounted && normPathname === page.path ? styles.activeMenu : ""}`}
            onClick={onClose}
          >
